...
print(net[2].bias.data)  # 这里显示的是数值本身
...
def block2():
    net = nn.Sequential()
    for i in range(4):
        net.add_module(f'block {i}', block1())  # 使用类似字典的方式添加block，这里是添加了4次block1()结构
    return net
...
# net.apply(init_normal)  # .apply() 遍历所有module，对其进行处理

def init_constant(m):
    if type(m) == nn.Linear:
        nn.init.constant_(m.weight, 1)  # 将权重全部初始化为1常数
        nn.init.zeros_(m.bias)
...
net = nn.Sequential(nn.Linear(4, 8), nn.ReLU(), shared, nn.ReLU(), shared, nn.ReLU(), nn.Linear(8, 1))  # 两个shared层初始化的时候是共享权重的
