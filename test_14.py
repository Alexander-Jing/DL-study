...
# 数值稳定性和relu()函数
# 数值稳定性：希望每一层的输出和反向的梯度都在一个合理的范围内
# 选择激活函数
# 假设：
# 每一层的输出和梯度都看做随机变量，独立同分布，让它们的均值和方差都保持一致
# 这样的设定可以方便优化，缩小优化的范围
# 结合MLP假设：
# 每一层的每一个神经元的权重都是独立同分布的，并且和输入是独立的
# 没有激活函数的前提下：
# Xavier：合适的的权重初始化的方法，依据输入层的数量进行设定每一层的每一个神经元权重的初始值（均值为0，方差依据输入层的数量设定）
# 有激活函数的前提下：
# 激活函数只有是线性并且是纯粹的y=x的情况，才会出现原来的输出经过激活函数之后依旧保持相同的方差和均值
# 而事实上，relu()和tanh()函数，都是非常好的在0附近是近似线性的，泰勒展开就可以了

# 上面两部分结合，其实就是：均值为0，依据输入规模设定权重方差；激活函数要在0处近似线性；这样可以方便优化
